...
namespace ANYI_Cafe.App_Class
{
    public static class Cart
    {
        ...
        /// <param name="buyQty">數量</param>
        public static void AddCarts(string productNo, string size_no, int buyQty)
        {
            using (anyicafeEntities db = new anyicafeEntities())
            {
                string str_spec = "";
        ...
        /// 消費者付款
        /// </summary>
        public static int CartPayment(cvmOrdersViewModel model)
        {
            int int_order_id = 0;
            OrderNo = CreateNewOrderNo(model);
        ...
        private static string CreateNewOrderNo(cvmOrdersViewModel model)
        {
            Shop.OrderID = 0;
            Shop.OrderNo = "0";
            string str_order_no = "";
            string str_guid = Guid.NewGuid().ToString().Substring(0, 25).ToUpper();
            string address = $"{model.City}{model.District}{model.Road}{model.Msg}";
            using (anyicafeEntities db = new anyicafeEntities())
            {
        ...
        /// 取得購物車資訊
        /// </summary>
        /// <param name="mno">如果有登入，傳入UserNo。沒登入，傳入LotNo</param>
        public static List<Carts> GetCartList(string mno)
        {
            using (anyicafeEntities db = new anyicafeEntities())
            {
                var data = db.Carts.Where(m => m.user_no == mno || m.lot_no == mno).ToList();
                return data;
            }
        }
